...
export type TipoPlantilla = "CONTRATO_MARCO" | "ORDEN_COMPRA" | "CARTA_ADJUDICACION";
...
/**
 * The award letter for one supplier, generated by the backend from the
 * company's letter template or Procurex's default format: PDF, or Word when
 * the PDF converter is unavailable. Downloaded, not opened: a tab opened after
 * the request would be caught by popup blockers.
 */
export async function descargarCartaAdjudicacion(requerimientoId: string, adjudicacionId: string) {
  const res = await api.get<Blob>(`/adjudicacion/${requerimientoId}/carta`, { params: { adjudicacionId }, responseType: "blob" });
  const nombre = nombreDe(res.headers["content-disposition"] as string | undefined, "carta-adjudicacion.pdf");
  entregar(res.data, nombre, false);
}

/** The supplier's copy of its award letter, in the buyer's format. */
export async function descargarMiCartaAdjudicacion(requerimientoId: string) {
  const res = await api.get<Blob>(`/ofertas/mine/${requerimientoId}/carta`, { responseType: "blob" });
  entregar(res.data, nombreDe(res.headers["content-disposition"] as string | undefined, "carta-adjudicacion.pdf"), false);
}
