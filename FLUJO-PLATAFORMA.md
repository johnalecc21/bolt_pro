...
```
/                    → Landing (qué es Procurex, precios y acceso a los portales)
...
6. **Adjudicación** (pestaña *Adjudicación*) — una tarjeta por proveedor adjudicado con sus líneas, condiciones, vista previa de la PO y botón **Carta de adjudicación** (PDF con la plantilla de carta de la empresa o, sin ella, el formato de Procurex con los datos de la empresa), más el ahorro sobre el total y los ítems desiertos. Confirmas la decisión (pasa a *Adjudicado* y se notifica una vez a cada ganador) → cada contrato se firma por separado: si supera el umbral legal de su moneda (≈ USD 50.000: 200M COP, 1M MXN, 190K PEN, 47M CLP, 280K BRL) se bloquea hasta marcar "revisión legal" → **"Firmar y generar contrato"**: registra la firma, crea el Contrato con sus hitos, notifica a ese ganador; cuando se firma el **último** contrato el proceso pasa a *En cumplimiento* y (según el checkbox) se notifica a quienes no ganaron nada. No se puede firmar dos veces. Mientras un proceso dividido tiene algún contrato firmado, el presupuesto cuenta lo firmado (no el estimado).
...
9. **Cuentas por pagar** (`/cliente/pagos`, Comprador, Admin y CFO) — cada pago liberado por un hito, con la factura que radicó el proveedor. Filtros *Requieren acción / Por pagar / Pagados / Todos*, búsqueda y totales (por pagar, vencido, vence en 30 días, facturas por revisar). Por pago:
   ...
   - **Registrar el pago** (fecha, referencia bancaria y soporte opcional) — solo con factura aprobada; solo Admin o CFO; no se registra dos veces.
...
3. **Procesos** (`/proveedor/procesos`) — una sola pantalla para todo proceso en el que puede participar, con tres pestañas:
   - **Nuevos**: las invitaciones sin responder y los procesos abiertos en la red de sus categorías. No hay paso de "aceptar": el proveedor revisa el requerimiento y pulsa **Preparar oferta** (guardar el primer borrador cuenta como aceptar) o **No participaré**. Quien declinó puede cambiar de idea mientras el proceso siga abierto.
   - **Participando**: sus ofertas en borrador o enviadas en procesos que siguen abiertos, con monto y estado. Si el comprador inicia una negociación aparece **Subasta en vivo**, donde ve **solo su posición** (nunca montos ni identidad de los demás), conectado por WebSocket a la misma sala del comprador. Cada oferta se completa en su formulario estandarizado; si el requerimiento tiene ítems cotiza **precio unitario por línea** y el total lo calcula el servidor. También puede preguntar en la sección de Q&A del proceso.
   - **Terminados**: procesos ganados y perdidos con la retroalimentación. En un proceso adjudicado por ítems, gana si le adjudicaron alguna línea (se indica que fue parcial) y figura como perdido solo cuando todos los contratos del proceso están firmados sin él.
   Las direcciones antiguas (`/proveedor/invitaciones`, `/oportunidades`, `/ofertas`, `/subasta`, `/historial`) redirigen a la pestaña correspondiente.
...
9. **Perfil de Empresa** — certificaciones, usuarios con acceso al portal y enlace a su **vitrina pública** (`/vitrina/:id`, sin login) para compartir con prospectos.
   - **Vitrina pública** (pestaña de *Perfil y vitrina*, `/proveedor/perfil?vista=vitrina`) — el proveedor arma su vitrina: presentación, correo/teléfono comercial, video (YouTube/Vimeo), galería de hasta 20 imágenes, brochures y catálogos en PDF (hasta 5 de cada uno) y un catálogo de productos/servicios con foto y precio de referencia. Ve cuántas visitas tiene su vitrina. Los compradores la encuentran en el Directorio, que también busca por descripción y por productos del catálogo.
10. **Carta de adjudicación** (en Historial) — una vez confirmada la adjudicación, el ganador descarga la misma carta que ve el comprador, con el formato de la empresa compradora.
11. **Evaluaciones de desempeño** (en Historial) — lo que sus clientes calificaron por contrato, con aviso de plan de mejora bajo 60/100.
12. **Mi desempeño** (`/proveedor/desempeno`) — la analítica del proveedor, solo con sus propios datos: monto adjudicado, tasa de éxito y de respuesta, embudo de invitaciones, ventas por mes, cliente y categoría, cumplimiento de hitos, evaluaciones, cobros (por cobrar, vencidos, próximos 30 días), contratos por vencer y visitas a su vitrina, con variación contra el período anterior y hallazgos. Filtros de período, moneda, cliente y categoría; exporta informe PDF, Excel y CSV. En los procesos perdidos ve su **posición por precio y la brecha con el precio adjudicado solo si la empresa compradora lo activó**; nunca ve el nombre del ganador ni precios de otros proveedores (y lo mismo aplica en el feedback del Historial).
...
- **Plantillas Word**:
  - La empresa sube su formato (.docx) con marcadores como `{{proveedor.nit}}`, `{{contrato.valorEnLetras}}` o una tabla `{{#lineas}}…{{/lineas}}`. Puede partir del **formato de Procurex** de cada tipo (botón *Formato de Procurex (Word)*), editarlo con su membrete y su redacción y subirlo como propio.
  ...
  - Hay una plantilla activa por tipo (orden de compra, contrato marco o carta de adjudicación). Una plantilla de categoría tiene prioridad sobre la general.
...
- **Carta de adjudicación**: se genera en el servidor al pedirla (comprador en la adjudicación, proveedor ganador en su historial), con la plantilla de carta activa o el formato de Procurex. Usa los mismos marcadores más `adjudicacion.*` (fecha, proceso, alcance, estado); no se guarda como versión del contrato.
- **Marca y datos**:
...
- **Menú por etapas**: el del cliente es Inicio · Compras (Requerimientos, Procesos de compra, Aprobaciones) · Contratos · Finanzas · Directorio, con **Configuración** aparte al fondo. El del proveedor es Inicio · Procesos · Contratos y pagos · Mi empresa (Perfil y vitrina, Homologación, Mi desempeño).
  - Los grupos se despliegan y se recuerdan. Al entrar a una pantalla se abre su grupo, pero cualquier grupo se puede colapsar, incluido el actual (queda resaltado para saber dónde estás). Con el menú colapsado, cada grupo es un ícono que abre sus opciones.
  - Los **contadores** muestran lo que espera al usuario: aprobaciones por aprobar, facturas por revisar, procesos nuevos del proveedor (invitaciones sin responder más procesos de la red), pagos por facturar y la cola de homologación. Salen de `GET /navegacion/contadores` y se refrescan al navegar y cada minuto.
...
- **Ingreso**: los tres logins y el registro de proveedor comparten un diseño claro (logo, formulario y un enlace), con **Volver al inicio** siempre visible, también en celular.
- **Cargas**: mientras una pantalla o ruta carga se muestra el logo animado de Procurex (versión clara u oscura según el tema, y quieto si el sistema pide reducir movimiento); las tablas siguen usando esqueletos.
...
- **Pagos**: el registro del pago es manual (fecha, referencia y soporte) o llega del ERP por la API de entrada; no hay integración bancaria ni validación de la factura electrónica ante la DIAN/SAT.
